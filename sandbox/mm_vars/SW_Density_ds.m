function d_rho_d_s = SW_Density_ds(T,uT,S,uS,P,uP)
    ...
    d_D_rho_d_s = b(1) + b(2).*T + b(3).*T.^2 + b(4).*T.^3 + 2*b(5)*s.*T.^2;
    ...
    d_F_P_d_s = ((P-P0).*(d(1) + d(2)*T + d(3)*T.^2)./1000).*( 0.5*(P.^2-P0.^2).*d(4)/1000).*F_P;
    
    rho = rho_sw_sharq.*F_P;
    
    d_rho_d_s = (d_rho_w_d_s + d_D_rho_d_s )* F_P + d_F_P_d_s * (rho_w + D_rho);
