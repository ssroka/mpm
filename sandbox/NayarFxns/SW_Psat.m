...
if ~isequal((T<0)+(T>180),zeros(size(T)))
    fprintf('Temperature is out of range for vapor pressure function 0<T<180 C\n');
    %         warning('Temperature is out of range for vapor pressure function 0<T<180 C');
end

if ~isequal((S<0)+(S>160),zeros(size(S)))
%     fprintf('Salinity is out of range for vapor pressure function 0<S<160 g/kg\n');
    %         warning('Salinity is out of range for vapor pressure function 0<S<160 g/kg');
end
