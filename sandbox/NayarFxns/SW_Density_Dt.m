function  [d_rho_dS,  d_rho_dT] = SW_Density_Dt(T,uT,S,uS,P,uP)
    % SW_Density    Density of seawater, modified to output temperature and salinity derivatives
    %=========================================================================
    ...
    %
    % VALIDITY: (1) 0 < T < 180 C; 0 < S < 150 g/kg; 0.1 < P = P0 < 1 MPa
    %           (2) 0 < T < 180 C; 0 < S < 56 g/kg; P_sat < P < 12 MPa
    %           (3) 0 < T < 180 C; 56 < S < 150 g/kg; P_sat < P < 12 MPa
    %
    % ACCURACY: (1) 0.14%
    %           (2) 0.14%
  ...
  ds_dS = 1/1000;
    ...
    d_rho_w_dT = a(2) + 2*a(3)*T.^1 + 3*a(4)*T.^2 +4*a(5)*T.^3;
    d_rho_w_ds = 0;

    D_rho = b(1)*s + b(2)*s.*T + b(3)*s.*T.^2 + b(4)*s.*T.^3 + b(5)*s.^2.*T.^2;
    d_D_rho_dT = b(2)*s + 2*b(3)*s.*T.^1 +3*b(4)*s.*T.^2 + 2*b(5)*s.^2.*T.^1;
    d_D_rho_ds = b(1) + b(2)*T + b(3)*T.^2 + b(4)*T.^3 + 2*b(5)*s.^1.*T.^2;
    
    rho_sw_sharq        = rho_w      + D_rho;
    d_rho_sw_sharq_dT   = d_D_rho_dT + d_rho_w_dT;
    d_rho_sw_sharq_ds   = d_D_rho_ds + d_rho_w_ds;
    ...
    d_F_P_dT = (  c(2) + 2*c(3)*T.^1 + 3*c(4)*T.^2 + 4*c(5)*T.^3 + 5*c(6)*T.^4 + S.*(  d(2) + 2*d(3)*T.^1)) + ...
        0.5*(P.^2-P0.^2).*(c(8) + 3*c(9)*T.^2)*...
	exp( (P-P0).*(c(1) + c(2)*T + c(3)*T.^2 + c(4)*T.^3 + c(5)*T.^4 + c(6)*T.^5 + S.*(d(1) + d(2)*T + d(3)*T.^2)) + ...
        0.5*(P.^2-P0.^2).*(c(7) + c(8)*T + c(9)*T.^3   + d(4)*S));

    d_F_P_dS = exp( (P-P0).*((d(1) + d(2)*T + d(3)*T.^2)) + ...
        0.5*(P.^2-P0.^2).*(d(4)));

    rho = rho_sw_sharq.*F_P;
    d_rho_dT = d_rho_sw_sharq_dT.*F_P+rho_sw_sharq.*d_F_P_dT; % the derivative of density in kg/m^s wrt temperature in deg C
    d_rho_dS = (d_rho_sw_sharq_ds*ds_dS).*F_P+rho_sw_sharq.*d_F_P_dS;% the derivative of density in kg/m^s wrt salinity in psu
