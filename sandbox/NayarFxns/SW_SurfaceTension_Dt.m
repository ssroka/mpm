function [d_sigma_dT, d_sigma_dS] = SW_SurfaceTension_Dt(T,uT,S,uS)
    % SW_SurfaceTension    Surface tension of seawater
    %=========================================================================
    % USAGE:  sigma = SW_SurfaceTension_Dt(T,uT,S,uS)
    %
    % DESCRIPTION:
    %   Adapted from Nayar function to output items for taking a time derivative
    %   of surface temsion.
    %   Derivatives are in temperatures of degrees Celsius and Salinity in ppt
    ...
    % let alpha1 = 1-(T+273.15)/647.096, and let alpha2 = 0.625*(1-(T+273.15)/647.096)
    % such that d_sigma_w_dT= d sigma_w / d alpha1 + d alpha1 / dT + d sigma_w / d alpha2 + d alpha2 / dT
    d_sigma_w_dT = 235.8*(1.256*(1/647.096).^0.256).*(-1/647.096).*(1-0.625*(1-((T+273.15)/647.096)))+...
                   235.8*((1-((T+273.15)/647.096)).^1.256).*(-1).*(-0.625/647.096);
    d_sigma_w_dS = 0;
    ...
    d_sigma_dT=d_sigma_w_dT.*(1+(a(1)*S+a(2)*S.*T))+ sigma_w.*(a(2)*S);
    d_sigma_dS=d_sigma_w_dS.*(1+(a(1)*S+a(2)*S.*T))+ sigma_w.*(a(1)+a(2).*T);
end
